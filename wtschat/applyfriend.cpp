...
void ApplyFriend::SlotApplySure()
{
    ...
    auto uid = UserMgr::GetInstance()->GetUid();
     qDebug()<<"GetInstance()->GetUid()" ;
    jsonObj["uid"] = uid;
    auto name = ui->name_ed->text();
     qDebug()<<"ui->name_ed->text()" ;
    if(name.isEmpty()){
    ...
    auto bakname = ui->back_ed->text();
     qDebug()<<"ui->back_ed->text()" ;
    if(bakname.isEmpty()){
        bakname = ui->back_ed->placeholderText();
    }
 qDebug()<<"ui->back_ed->placeholderText()" ;
    jsonObj["bakname"] = bakname;
    qDebug()<<bakname;
    jsonObj["touid"] = _si->_uid;
    qDebug()<< _si->_uid;
qDebug()<<" jsonObj = _si->_uid;" ;
    QJsonDocument doc(jsonObj);
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact);

    //发送tcp请求给chat server
    qDebug()<<"发送tcp请求给chat server前" ;
    emit TcpMgr::GetInstance()->sig_send_data(ReqId::ID_ADD_FRIEND_REQ, jsonData);//ID_NOTIFY_ADD_FRIEND_REQ
     qDebug()<<"发送tcp请求给chat server后" ;
    this->hide();
    deleteLater();
}
